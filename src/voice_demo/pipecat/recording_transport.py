...
def _resample_pcm16(data: bytes, src_rate: int, dst_rate: int) -> bytes:
    if not data or src_rate == dst_rate or not src_rate:
        return data
...
class ConversationRecorder:
    ...
    def save_recording(self) -> None:
        ...
        user_chunks = [(t, _resample_pcm16(d, r or target, target)) for t, d, r in user]
        agent_chunks = [(t, _resample_pcm16(d, r or target, target)) for t, d, r in agent]
